...
#define  SERIAL_STATE_NONE                                 0u //¿ÕÏÐ
#define  SERIAL_STATE_CLOSED                               1u //¹Ø±Õ
#define  SERIAL_STATE_CLOSING                              2u //ÕýÔÚ¹Ø±Õ
#define  SERIAL_STATE_OPENED                               3u //´ò¿ª
                        ...
                        SERIAL_ERR      *perr)
{
    ...
    CPU_INT08U     *pbuf_data; //Èç¹ûÊ¹ÄÜ·¢ËÍºÍ½ÓÊÜ»º³åÇø
#endif
#if (SERIAL_CFG_RD_BUF_EN       == DEF_ENABLED) || (SERIAL_CFG_WR_BUF_EN       == DEF_ENABLED) || (SERIAL_CFG_TX_DESC_NBR >  0)  
                                    //RD WR ENABLE
    CPU_SIZE_T      octets_reqd;   //×Ö½ÚÊýÊ¹ÄÜ  
    LIB_ERR         lib_err;       //library error
#endif
    CPU_SR_ALLOC();
...
*********************************************************************************************************
*/
//Set serial interface line driver
...
//´¦Àí½ÓÊÜÊý¾Ý
void  ClearSerialBufHandling (SERIAL_DEV  *pdev,
                         CPU_INT08U   datum)
{
    SERIAL_IF             *pif;
    SERIAL_IF_NBR          if_nbr;

    if_nbr =  pdev->IF_Nbr;            //µÚ¼¸¸ö¶Ë¿Ú
    pif    = &Serial_IF_Tbl[if_nbr];   //Êý¾Ý±£´æµÚ¼¸¸ö¶Ë¿Ú
...
void OvertimeSerialBufHandling (SERIAL_DEV  *pdev,
                           CPU_INT08U   datum)
{
    SERIAL_IF             *pif;
    SERIAL_IF_NBR          if_nbr;
                   ...
                   CPU_INT08U   datum)
{
    ...
    switch (pif->RdState) {  //¶ÁÈ¡×´Ì¬
        ...
        case SERIAL_RD_STATE_RD:                                /* --------------- RD DATA INTO USER BUF -------------- */
             pbuf = &(pif->RdUserBuf);//¶ÁÈ¡ÐÅÏ¢µ½ÓÃ»§Buffer
